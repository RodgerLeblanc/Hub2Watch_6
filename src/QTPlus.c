...
void qtp_init() {
		...
		/* Divider inverter layer (White bar) */
        GRect divider_frame = GRect( 5, 62, QTP_SCREEN_WIDTH-5-5, 2 );
        qtp_divider_layer = inverter_layer_create(divider_frame);
        layer_add_child(window_get_root_layer(qtp_window), inverter_layer_get_layer(qtp_divider_layer));
...
void qtp_deinit() {
		...
		inverter_layer_destroy(qtp_divider_layer);
